...
scoreN = 0
TimeBonusN = 0
Rekordsl = open('Rekords.json').read()
Rekords = json.loads(Rekordsl)
OVERbool = 0
OVERimg = pygame.font.Font('UF.otf', 64)
...
def RekordsUpdate():
    ...
    global OVERimg
    global scoreN
    Rekords.append(scoreN + TimeBonusN)
    Rekords = sorted(Rekords, reverse=1)[:3]
    OVERtxt = 'me ' + str(Rekords[0])
    OVERtxt1 = 'me ' + str(Rekords[1])
    OVERtxt2 = 'me ' + str(Rekords[2]) 
    OVER = OVERimg.render(OVERtxt, 1, (255,0,0))
    OVER1 = OVERimg.render(OVERtxt1, 1, (255,0,0))
    OVER2 = OVERimg.render(OVERtxt2, 1, (255,0,0))
    print(Rekords)
    return OVERtxt,OVERtxt1,OVERtxt2,OVER,OVER1,OVER2

# OVERtxt,OVERtxt1,OVERtxt2,OVER,OVER1,OVER2 = RekordsUpdate()
# add screen
...
while run:
    ...
    for i in range(enemynum):
        if enemyY[i] >= 500:
            OVERtxt,OVERtxt1,OVERtxt2,OVER,OVER1,OVER2 = RekordsUpdate()
            json.dump(Rekords, open('Rekords.json', 'w'))
            OVERbool = 1
            for i in range(enemynum):
    ...
    if OVERbool == 0:
        ...
        screen.blit(OVER1, (300, 250))
        screen.blit(OVER1, (300, 300))
